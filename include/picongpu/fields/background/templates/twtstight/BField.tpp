    ...
    namespace templates
    {
        ...
        namespace twtstight
        {
            ...
            BField::float_T BField::calcTWTSBy(float3_64 const& pos, float_64 const time) const
            {
                ...
                complex_T const rhom = math::sqrt(x2 + pmacc::math::cPow(-z - complex_T(0,0.5) * k * w02, static_cast<uint32_t>(2u)));
                ...
                (
                  math::exp(I * (omega0 * t - k * y * cosPhi)) * k * zeroOrder
                  * sinPhi
                  * (
                     - (
                        besselJ1const * (Xm * cosPolAngle * (float_T(1.0) + cosPhi_2)
            ...
            BField::float_T BField::calcTWTSBz(float3_64 const& pos, float_64 const time) const
            {
                ...
                (
                  ...
                  * (
                     ...
                     * (
                          sinPolAngle * (x * (float_T(2.0) * Xm - I * k * rhom2 * sinPhi) + cosPhi * (rhom2 - float_T(2.0) * Xm2 - I * k * rhom2 * Xm * sinPhi))
                        + cosPolAngle * (x * (float_T(2.0) * Xm + I * k * rhom2 * sinPhi) + cosPhi * (-rhom2 + float_T(2.0) * Xm2 + I * k * rhom2 * Xm * sinPhi))
                       )
                     + k * rhom * besselJ0const
                     * (
                        Xm * (-x + Xm * cosPhi) * sinPolAngle * sinPhi_2
                        - cosPolAngle * (x * Xm * sinPhi_2 + cosPhi * (float_T(-2.0) * rhom2 + Xm2 * sinPhi_2))
                       )
                    ) * psi0
                )
            ...
            BField::float_T BField::calcTWTSBx(float3_64 const& pos, float_64 const time) const
            {
                 ...
                 * (
                    ...
                    * (
                       ...
                       - sinPolAngle * (rhom2 + rhom2 * cosPhi_2 - x2 * sinPhi_2 + x * Xm * cosPhi * sinPhi_2)
                      )
                    + besselJ1const
                    * (
                         cosPolAngle * sinPhi
                       ...
                       * (
                          ...
                          + I * k * rhom2 * (-Xm + x * cosPhi) * sinPhi_2
                          + x * Xm * sin2Phi
                         )
                      )
                   ) * psi0
